...
class TestController extends Controller {
	...
	public function actionTestT() {
		...
		$data  = $excel::import([
			...
			// set this if your excel data with multiple worksheet, the index of array will be set with the sheet name. If this not set, the index will use numeric.
			'getOnlySheet'         => 'sheet1',
			// you can set this property if you want to get the specified sheet from the excel data with multiple worksheet.
		]);
	}
	...
	public function actionRegister() {
		...
		foreach ($users as $user) {
			$sinhVien = SinhVien::findOne(['email' => $user->email]);
			if ($user->info_id == null && $sinhVien !== null) {
				$user->updateAttributes([
					'info_id' => $sinhVien->id,
					'flags'   => User::FLAG_SINHVIEN,
				]);
